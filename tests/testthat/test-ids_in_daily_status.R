describe("All IDs defined in list are in daily status", {
  ...
  it("Check all ID's are in ids list", {
    ...
    expect_error(check_all_ids_in_list_are_in_daily_status(correct_daily_status_path, incorrect_traps_list_path), "🚨 ID not present in daily status table: 99-998")
  })
})
